...
const transformRestaurantData = (restaurant) => {
  ...
  return {
    ...
    createdAt: restaurant.created_at
      ? new Date(restaurant.created_at).toISOString()
      : null,
    updatedAt: restaurant.updated_at
      ? new Date(restaurant.updated_at).toISOString()
      : null,
...
const resolvers = {
  Query: {
    ...
    deliverer: async (_, {userId}, {supabase}) => {
      ...
      return {
        ...
        currentLocation:
          typeof data.current_location === "string"
    ...
    allDeliverers: async (_, __, {supabase}) => {
      ...
      return data.map((d) => ({
        ...
        currentLocation:
          typeof d.current_location === "string"
            ? JSON.parse(d.current_location)
            : d.current_location,
        zone: d.zone,
    ...
    restaurantsNearby: async (
      _,
      {latitude, longitude, maxDistance = 10.0},
      {supabase},
    ) => {
      try {
        ...
        return restaurantsWithDistance.map((restaurant) => {
          ...
          const ratings = restaurantOrders
            .map((order) => order.rating)
            .filter((r) => r !== null && r !== undefined);
          const averageRating =
            ratings.length > 0
      ...
      try {
        ...
        return (data || []).map((menuItem) => ({
          ...
          createdAt: menuItem.created_at
            ? new Date(menuItem.created_at).toISOString()
            : "Not provided",
          updatedAt: menuItem.updated_at
            ? new Date(menuItem.updated_at).toISOString()
            : "Not provided",
    ...
    orders: async (
      _,
      {userId, restaurantId, status, delivererId},
      {supabase},
    ) => {
      try {
        let query = supabase
        ...
        const userIds = [...new Set(orders.map((o) => o.user_id))].filter(
          Boolean,
        );
        const restIds = [...new Set(orders.map((o) => o.restaurant_id))].filter(
          Boolean,
        );
        const delivIds = [...new Set(orders.map((o) => o.deliverer_id))].filter(
          Boolean,
        );
        ...
        const delivMap = Object.fromEntries(
          (deliverers || []).map((d) => [d.user_id, d]),
        );
        const delivUserMap = Object.fromEntries(
          (delivUsers || []).map((u) => [u.id, u]),
        );
        ...
        return orders.map((order) => {
          ...
          if (delivererData) {
            deliverer = {
              ...
              currentLocation:
                typeof delivererData.current_location === "string"
    ...
    order: async (_, {id}, {supabase}) => {
      try {
        ...
        // Fetch related data
        const [itemsResult, userResult, restResult, delivResult] =
          await Promise.all([
    ...
    restaurantRatings: async (_, {restaurantId}, {supabase}) => {
      try {
        ...
        const userIds = [...new Set(ratings.map((r) => r.user_id))].filter(
          Boolean,
        );
        ...
        return ratings.map((rating) => {
          ...
          return {
            ...
            updatedAt: rating.updated_at
              ? new Date(rating.updated_at).toISOString()
              : null,
          };
        });
      } catch (err) {
    ...
    delivererStats: async (_, {delivererId}, {supabase}) => {
      try {
        ...
        if (todayError) {
          throw new Error(
            `Error fetching today's orders: ${todayError.message}`,
          );
        }

        // Calculate totals
        const totalEarnings = (allOrders || []).reduce(
          (sum, order) => sum + (order.deliverer_payout || 0),
          0,
        );
  ...
  Mutation: {
    ...
    updateMenuItem: async (_, {id, input}, {supabase}) => {
      try {
        ...
        if (input.isAvailable !== undefined)
          dbInput.is_available = input.isAvailable;
    ...
    updateOrderStatus: async (_, {id, status}, {supabase}) => {
      ...
      if (order && order.user_id) {
        ...
        await sendPushNotification(
          order.user_id,
          "Mise à jour de commande",
          `Votre commande est maintenant ${statusFrench}`,
          {type: "ORDER_UPDATE", orderId: order.id, screen: "OrderTracking"},
          supabase,
        );
      }
    ...
    confirmOrderDelivery: async (
      _,
      {orderId, token, delivererId},
      {supabase},
    ) => {
      ...
      await sendPushNotification(
        order.user_id,
        "Commande livrée",
        `Votre commande #${order.id} est livrée ✅`,
        {type: "ORDER_DELIVERED", orderId: order.id, screen: "OrderTracking"},
        supabase,
      );
    ...
    deleteOrder: async (_, {id}, {supabase}) => {
      try {
        ...
        if (order.status !== "COMPLETED" && order.status !== "CANCELLED") {
          throw new Error(
            "Seules les commandes terminées ou annulées peuvent être supprimées.",
          );
        }
        ...
        if (itemsError) {
          throw new Error(
            "Impossible de supprimer les articles de la commande.",
          );
        }
    ...
    updateOrderNote: async (_, {orderId, note}, {supabase}) => {
      try {
        ...
        if (error)
          throw new Error(`Error updating order note: ${error.message}`);
    ...
    updateOrderRating: async (_, {orderId, rating}, {supabase}) => {
      try {
        ...
        if (error)
          throw new Error(`Error updating order rating: ${error.message}`);
    ...
    updateOrderFeedback: async (_, {orderId, rating, note}, {supabase}) => {
      try {
        ...
        if (error)
          throw new Error(`Error updating order feedback: ${error.message}`);
    ...
    rateOrderComplete: async (_, {input}, {supabase}) => {
      try {
        ...
        if (order.status !== "COMPLETED")
          throw new Error("Seules les commandes terminées peuvent être notées");
  ...
  Order: {
    user: (parent) => {
      ...
      return {
        ...parent.user,
        phoneNumber:
          parent.user.phoneNumber || parent.user.phone_number || "Not provided",
        createdAt: parent.user.created_at
          ? new Date(parent.user.created_at).toISOString()
          : null,
      };
    },
  ...
  Restaurant: {
    rating: async (parent, _, {supabase}) => {
      try {
        ...
        if (error)
          throw new Error(`Error fetching order ratings: ${error.message}`);
  ...
  Deliverer: {
    ...
    averageRating: async (parent, _, {supabase}) => {
      try {
        ...
        if (error)
          throw new Error(`Error fetching deliverer ratings: ${error.message}`);
        if (!orders || orders.length === 0) return null;

        const sum = orders.reduce(
          (acc, order) => acc + (order.deliverer_rating || 0),
          0,
        );
    ...
    totalRatings: async (parent, _, {supabase}) => {
      try {
        ...
        if (error)
          throw new Error(`Error counting deliverer ratings: ${error.message}`);
